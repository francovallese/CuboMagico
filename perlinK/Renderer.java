...
private String notice = "Copyright 2001 Ken Perlin. All rights reserved.";
   ...
   */
   ...
   public static int pack(int r, int g, int b) 
   {
	   ...
	   //System.out.println("r:"+r +" g:"+g+" b:"+b+" ->:"+ret);
	   return ret;
   } 
   ...
   private synchronized void changeCamera(double theta, double phi) {
      ...
      camera.postMultiply(camtmp);
      /*
      Forma.theta = camera.get(0, 1);
      Forma.phi   = camera.get(1, 1);
      Forma.cm    = new Matrix();
      Forma.cm.copy(camera);
      Forma.xyz = getCameraPos();
      */
      if (isHeadsUp) 
      {
   ...
   private void renderWorld() {
      ...
      if(b_aliasing)
      {
    	 //System.out.println("ATTIVO");
    	  for(int k = 100*W; k<pix.length-100*W; k++)
	      {
	    	int x = k %W;
	      	int y = (k - x) / W;
	      	if(x>200 && x < W-200)
	      	{
	     		...
	     		rgb[0] = (pix[k] >> 16) & 255;
	  	        rgb[1] = (pix[k] >>  8) & 255;
	  	        rgb[2] = (pix[k]      ) & 255;
	  	        if(rgb[0] == 102 && rgb[1] == 102 && rgb[2] == 204)continue; // NO SFONDO
	  	        
	     		Geometry g = getGeometry(x,y);
	     		boolean bordo = false;
	     		
	      		if( g!= null && g.name.length() == 2 )  //  BORDI
	      		{
	      			bordo = true;
	      		}
	        	...
	        	Geometry gd = getGeometry(xd,yd);
	        	Geometry gs = getGeometry(xs,ys);
	        	Geometry ga = getGeometry(xa,ya);
	        	Geometry gb = getGeometry(xb,yb);
	        	Geometry gad = getGeometry(xad,yad);
	        	Geometry gas = getGeometry(xas,yas);
	        	Geometry gbd = getGeometry(xbd,ybd);
	        	Geometry gbs = getGeometry(xbs,ybs);
	        	int r  = 0;
        		int gg = 0;
        		int b  = 0;
	        	...
	        	if(bordo)
	        	{
	        		rgbs[0] = (pix[k-1] >> 16) & 255;
		  	        rgbs[1] = (pix[k-1] >>  8) & 255;
		  	        rgbs[2] = (pix[k-1]      ) & 255;
		  	        
	        		if(rgbs[0] == 102 && rgbs[1] == 102 && rgbs[2] == 204)
	        		{
	        			rgbd[0] = (pix[k+1] >> 16) & 255;
		      	        rgbd[1] = (pix[k+1] >>  8) & 255;
		      	        rgbd[2] = (pix[k+1]      ) & 255;
		      	        
		      	        
		      	        rgba[0] = (pix[k-W] >> 16) & 255;
		      	        rgba[1] = (pix[k-W] >>  8) & 255;
		      	        rgba[2] = (pix[k-W]      ) & 255;
		      	        
		      	        rgbb[0] = (pix[k+W] >> 16) & 255;
		      	        rgbb[1] = (pix[k+W] >>  8) & 255;
		      	        rgbb[2] = (pix[k+W]      ) & 255;
		      	        
		      	        rgbad[0] = (pix[k-W+1] >> 16) & 255;
		      	        rgbad[1] = (pix[k-W+1] >>  8) & 255;
		      	        rgbad[2] = (pix[k-W+1]      ) & 255;
		      	        
		      	        rgbas[0] = (pix[k-W-1] >> 16) & 255;
		      	        rgbas[1] = (pix[k-W-1] >>  8) & 255;
		      	        rgbas[2] = (pix[k-W-1]      ) & 255;
		      	        
		      	        
		      	        rgbbd[0] = (pix[k+W+1] >> 16) & 255;
		      	        rgbbd[1] = (pix[k+W+1] >>  8) & 255;
		      	        rgbbd[2] = (pix[k+W+1]      ) & 255;
		      	        
		      	        rgbbs[0] = (pix[k+W-1] >> 16) & 255;
		      	        rgbbs[1] = (pix[k+W-1] >>  8) & 255;
		      	        rgbbs[2] = (pix[k+W-1]      ) & 255;
		                	
		      	    	r  = (rgb[0]+rgba[0]+rgbb[0]+rgbd[0]+3*rgbs[0]+rgbad[0]+3*rgbas[0]+rgbbd[0]+3*rgbbs[0])/15; 
		        		gg = (rgb[1]+rgba[1]+rgbb[1]+rgbd[1]+3*rgbs[1]+rgbad[1]+3*rgbas[1]+rgbbd[1]+3*rgbbs[1])/15;
		        		b  = (rgb[2]+rgba[2]+rgbb[2]+rgbd[2]+3*rgbs[2]+rgbad[2]+3*rgbas[2]+rgbbd[2]+3*rgbbs[2])/15;
		        		
		      	      	pix[k]   = pack((int)(r), (int)(gg), (int)(b));
	        	   ...
	        	   )
	        	{
		        			   ...
		        			   
		        	{
	      	        for(int p= 0 ; p< 3; p++)
	      	        {
      	        	rgb[0] = (pix[k-1] >> 16) & 255;
		  	        rgb[1] = (pix[k-1] >>  8) & 255;
		  	        rgb[2] = (pix[k-1]      ) & 255;
		  	        	
	      	        rgbd[0] = (pix[k-1+1] >> 16) & 255;
	      	        rgbd[1] = (pix[k-1+1] >>  8) & 255;
	      	        rgbd[2] = (pix[k-1+1]      ) & 255;
	      	        
	      	        rgbs[0] = (pix[k-1-1] >> 16) & 255;
	      	        rgbs[1] = (pix[k-1-1] >>  8) & 255;
	      	        rgbs[2] = (pix[k-1-1]      ) & 255;
	      	        
	      	        rgba[0] = (pix[k-1-W] >> 16) & 255;
	      	        rgba[1] = (pix[k-1-W] >>  8) & 255;
	      	        rgba[2] = (pix[k-1-W]      ) & 255;
	      	        
	      	        rgbb[0] = (pix[k-1+W] >> 16) & 255;
	      	        rgbb[1] = (pix[k-1+W] >>  8) & 255;
	      	        rgbb[2] = (pix[k-1+W]      ) & 255;
	      	        
	      	        rgbad[0] = (pix[k-1-W+1] >> 16) & 255;
	      	        rgbad[1] = (pix[k-1-W+1] >>  8) & 255;
	      	        rgbad[2] = (pix[k-1-W+1]      ) & 255;
	      	        
	      	        rgbas[0] = (pix[k-1-W-1] >> 16) & 255;
	      	        rgbas[1] = (pix[k-1-W-1] >>  8) & 255;
	      	        rgbas[2] = (pix[k-1-W-1]      ) & 255;
	      	        
	      	        
	      	        rgbbd[0] = (pix[k-1+W+1] >> 16) & 255;
	      	        rgbbd[1] = (pix[k-1+W+1] >>  8) & 255;
	      	        rgbbd[2] = (pix[k-1+W+1]      ) & 255;
	      	        
	      	        rgbbs[0] = (pix[k-1+W-1] >> 16) & 255;
	      	        rgbbs[1] = (pix[k-1+W-1] >>  8) & 255;
	      	        rgbbs[2] = (pix[k-1+W-1]      ) & 255;
	                
	      	     	r  = (rgb[0]+rgba[0]+rgbb[0]+rgbd[0]+3*rgbs[0]+rgbad[0]+3*rgbas[0]+rgbbd[0]+3*rgbbs[0])/15; 
	        		gg = (rgb[1]+rgba[1]+rgbb[1]+rgbd[1]+3*rgbs[1]+rgbad[1]+3*rgbas[1]+rgbbd[1]+3*rgbbs[1])/15;
	        		b  = (rgb[2]+rgba[2]+rgbb[2]+rgbd[2]+3*rgbs[2]+rgbad[2]+3*rgbas[2]+rgbbd[2]+3*rgbbs[2])/15;
	        		
      	      		pix[k-1]   = pack((int)(r), (int)(gg), (int)(b));
	      	    	...
	      	    	r  = (rgb[0]+rgba[0]+rgbb[0]+rgbd[0]+rgbs[0]+rgbad[0]+rgbas[0]+rgbbd[0]+rgbbs[0])/9; 
	        		gg = (rgb[1]+rgba[1]+rgbb[1]+rgbd[1]+rgbs[1]+rgbad[1]+rgbas[1]+rgbbd[1]+rgbbs[1])/9;
	        		b  = (rgb[2]+rgba[2]+rgbb[2]+rgbd[2]+rgbs[2]+rgbad[2]+rgbas[2]+rgbbd[2]+rgbbs[2])/9;
	      	        ...
	      	        if(gd.name.length()  == 2)
	      	        {
		        		...
		        		b  = (rgb[2]+rgba[2]+rgbb[2]+rgbd[2]+rgbs[2]+rgbad[2]+rgbas[2]+rgbbd[2]+rgbbs[2])/9;
		      	        ...
		      	        rgbbs[2] = (pix[k+1+W-1]      ) & 255;
		      	     	...
		      	     	r  = (rgb[0]+rgba[0]+rgbb[0]+3*rgbd[0]+rgbs[0]+3*rgbad[0]+rgbas[0]+3*rgbbd[0]+rgbbs[0])/15; 
		        		gg = (rgb[1]+rgba[1]+rgbb[1]+3*rgbd[1]+rgbs[1]+3*rgbad[1]+rgbas[1]+3*rgbbd[1]+rgbbs[1])/15;
		        		b  = (rgb[2]+rgba[2]+rgbb[2]+3*rgbd[2]+rgbs[2]+3*rgbad[2]+rgbas[2]+3*rgbbd[2]+rgbbs[2])/15;
		  	      ...
		  	      if(ga.name.length()  == 2)
		  	      {
      	        	...
      	        	rgb[0] = (pix[k-W] >> 16) & 255;
		  	        rgb[1] = (pix[k-W] >>  8) & 255;
		  	        rgb[2] = (pix[k-W]      ) & 255;
		  	        	
	      	        rgbd[0] = (pix[k-W+1] >> 16) & 255;
	      	        rgbd[1] = (pix[k-W+1] >>  8) & 255;
	      	        rgbd[2] = (pix[k-W+1]      ) & 255;
	      	        
	      	        rgbs[0] = (pix[k-W-1] >> 16) & 255;
	      	        rgbs[1] = (pix[k-W-1] >>  8) & 255;
	      	        rgbs[2] = (pix[k-W-1]      ) & 255;
	      	        
	      	        rgba[0] = (pix[k-W-W] >> 16) & 255;
	      	        rgba[1] = (pix[k-W-W] >>  8) & 255;
	      	        rgba[2] = (pix[k-W-W]      ) & 255;
			      	        
	      	        rgbb[0] = (pix[k-W+W] >> 16) & 255;
	      	        rgbb[1] = (pix[k-W+W] >>  8) & 255;
	      	        rgbb[2] = (pix[k-W+W]      ) & 255;
	      	        
	      	        rgbad[0] = (pix[k-W-W+1] >> 16) & 255;
	      	        rgbad[1] = (pix[k-W-W+1] >>  8) & 255;
	      	        rgbad[2] = (pix[k-W-W+1]      ) & 255;
	      	        
	      	        rgbas[0] = (pix[k-W-W-1] >> 16) & 255;
	      	        rgbas[1] = (pix[k-W-W-1] >>  8) & 255;
	      	        rgbas[2] = (pix[k-W-W-1]      ) & 255;
	      	        
	      	        
	      	        rgbbd[0] = (pix[k-W+W+1] >> 16) & 255;
	      	        rgbbd[1] = (pix[k-W+W+1] >>  8) & 255;
	      	        rgbbd[2] = (pix[k-W+W+1]      ) & 255;
	      	        
	      	        rgbbs[0] = (pix[k-W+W-1] >> 16) & 255;
	      	        rgbbs[1] = (pix[k-W+W-1] >>  8) & 255;
	      	        rgbbs[2] = (pix[k-W+W-1]      ) & 255;
	                
	      	     	r  = (rgb[0]+3*rgba[0]+rgbb[0]+rgbd[0]+rgbs[0]+3*rgbad[0]+3*rgbas[0]+rgbbd[0]+rgbbs[0])/15; 
	        		gg = (rgb[1]+3*rgba[1]+rgbb[1]+rgbd[1]+rgbs[1]+3*rgbad[1]+3*rgbas[1]+rgbbd[1]+rgbbs[1])/15;
	        		b  = (rgb[2]+3*rgba[2]+rgbb[2]+rgbd[2]+rgbs[2]+3*rgbad[2]+3*rgbas[2]+rgbbd[2]+rgbbs[2])/15;
	        		
	  	      		pix[k-W]   = pack((int)(r), (int)(gg), (int)(b));	
	        		...
	        		b  = (rgb[2]+rgba[2]+rgbb[2]+rgbd[2]+rgbs[2]+rgbad[2]+rgbas[2]+rgbbd[2]+rgbbs[2])/9;
	      	        ...
	      	        rgbbs[2] = (pix[k+W+W-1]      ) & 255;
	      	     	...
	      	     	r  = (rgb[0]+rgba[0]+3*rgbb[0]+rgbd[0]+rgbs[0]+rgbad[0]+rgbas[0]+3*rgbbd[0]+3*rgbbs[0])/15; 
	        		gg = (rgb[1]+rgba[1]+3*rgbb[1]+rgbd[1]+rgbs[1]+rgbad[1]+rgbas[1]+3*rgbbd[1]+3*rgbbs[1])/15;
	        		b  = (rgb[2]+rgba[2]+3*rgbb[2]+rgbd[2]+rgbs[2]+rgbad[2]+rgbas[2]+3*rgbbd[2]+3*rgbbs[2])/15;
   ...
   private void renderGeometry(Geometry s) {
      ...
      //System.out.println("RECTANGULAR MESH "+ m);
      //if (lod > 1 && m >= 2) {
      if (lod > 1 && m >= 40) {
         ...
         for (int J = 0; J <= N - 1 - lod; J += lod)
            for (int I = 0; I <= M - 1 - lod; I += lod) {
               ...
               //System.out.println("render renderGeometry 1");
               fillAndClipTriangle(s, a, b, c);
               //System.out.println("render renderGeometry 2");
      ...
      // ALL OTHER CASES
         ...
         for (int j = 0; j < s.faces.length; j++) {
            ...
            if (f != null)
               ...
               {
            	  /////////System.out.println("render renderGeometry 3 " + s.name);
   ...
   */
   ...
   private void fillAndClipTriangle(Geometry s, int iA, int iB, int iC) 
   {
      ...
      if (t1[iC][2] + epsilonPlane > FL)
         clipping += 4;
/////////////System.out.println("renderer fillAndClipTriangle "+s.name + " clipping "+clipping);
      switch (clipping) {
         case (0) :
   ...
   private void clip2Vert(Geometry s, int clip1, int clip2, int c) {
      ...
      // 4. fill triangle
	   //System.out.println("renderer clip2Vert");
      double[] n1 = new double[s.verticedepth];
      double[] n2 = new double[s.verticedepth];
...
private static double lerp(double t, double a, double b) {
 ...
 //     ti[1] = H/2 - W * (cY/FL + pz * (ti[1] - cY)) / FOV;
   ...
   {
	   /////////////System.out.println("renderer fillTriangle 3 interi");
	   /*
	   System.out.println(""+iA + " " +iB+ " "+iC);
	   for(int g=0; g<t.length;g++)
       {
     	  System.out.print( " T ["+g+"]" + t[g][0]+ " "+ t[g][1]+ " "+ t[g][2]+ " "+ t[g][3]+ " "+ t[g][4]+ " "+ t[g][5]);
   ...
   {
	   ///////////////System.out.println("renderer fillTriangle");
      if (same(A, B) || same(B, C))
      {
          ...
          for(int g=0; g<6;g++)
          {
        	  ...
        	  System.out.print( " C "+ C[g]);
          }
          */
         return;
      }
      
      boolean bf = backfacing(A, B, C);
      if(s.name.startsWith("et"))
      {    	  
    	  Faccetta fac = (Faccetta)s.getParent();
    	  fac.set_visible(!bf);    	 
      }
      /*
      if(s.name.startsWith("TRI"))
      {
    	  String[] xx = s.name.split("[_]",0);
    	  String cubetto = xx[1];
    	  String faccia  = xx[2];
    	  Faccetta fac = (Faccetta)s.getParent();
    	  fac.set_visible(!bf);
    	  //fac.set_TA(A);
          //System.out.println(bf+" cubetto "+cubetto + " faccia "+faccia + " nc="+fac.indice + " nf="+fac.numf+ " visibile " + fac.get_visible());
    	  //System.out.println(bf+ Arrays.toString(A) + " " + Arrays.toString(B)+ " "+Arrays.toString(C));
      }
      */
      /*
      System.out.println("backfacing= " + bf);
   ...
   private void fillTrapezoid(int A[], int B[], int C[], int D[], Geometry s) {
      ...
      for (int y = yLo; y < yHi; y++) 
      {
         ...
         if(s.name.startsWith("et"))
         {
        	 ...
        	 //System.out.println(s.name+" x?:"+ixL+ " y?:"+y + " z:"+ z);
         }
         //else
         {
   ...
   */
   ...
   private synchronized static void renderVertex(int ix, int iy, int iz, double x, double y, double z, double v[], Material m) {
      ...
      if (tableMode && m.tableMode)
      {
    	  //System.out.println("RGB: " + v[3] + " " + v[4]+ " " + v[5]);
...
private static double computeHilite(double x, double y, double z, double nx, double ny, double nz, double L[]) {
   ...
   public int pix[]; // THE FRAME BUFFER
   ...
   private boolean b_aliasing = false;
   public void set_aliasing(boolean b_aliasing)
   {
	   this.b_aliasing = b_aliasing;
   }
   public boolean get_aliasing()
   {
	   return b_aliasing;
   }
