...
public class RenderJPanel extends JPanel implements Runnable 
{
	 ...
	 * 
	 */
	private static final long serialVersionUID = 2966622100599472609L;
   ...
   */
   public void run() 
   {
	   @SuppressWarnings("unused") // NON ANDREBBE TOLTO?
      int o = 0;
      while (running) {
         renderFrame();
   ...
   public void renderFrame() {
      ...
      if (isDamage) {
         ...
         //System.out.println("RENDER"+mat+" lod:"+renderer.lod);
         renderer.render();
   ...
   private synchronized void recalculateSize(int currentWidth, int currentHeight) {
      ...
      W = currentWidth;
      H = currentHeight;
      //System.out.println("recalculateSize "+H);
   ...
   */
   ...
   {
	  //System.out.println("paintComponent su RenderJPanel");
      ...
      //System.out.println("passo"+aliasing);
      ...
      if(aliasing)
      {
    	  ...
    	  for (int y = 0; y < H; y++) 
    	  {
    		  ...
    		  {
    			  int blue = (x * 255) / (W - 1);
    			  //int val = (255 << 24) | (red << 16) | blue;
    			  int val = (255 << 26) | (red << 18) | blue;
    			  pix1[index++] = val;
    		  }
    	  }
    	  
    	  MemoryImageSource mis1 = new MemoryImageSource(W, H, pix1, 0, W);
          mis1.setAnimated(true);
          Image im1 = createImage(mis1);
    	  
    	 
    	  g.drawImage(im1, insets.left, insets.top, null);
    	  g.setColor(Color.YELLOW);
    	  g.drawString("DA FARE...", W/2, H/2);
      ...
      
      {
    	  ...
    	  g2d.drawImage(im, insets.left, insets.top, null);
      }
      */
...
/**
   ...
   */
   ...
   /**
   			...
   			@return true
   		*/
   public boolean mouseMove(MouseEvent event) // MouseEvent event 
   {
	   int x = event.getX();
	   int y = event.getY();
	if (seeMaterial) {
   ...
   /**
   	...
   	@return true
   */
   public boolean mouseDown(MouseEvent event) // MouseEvent event 
   {
	   int x = event.getX();
	   int y = event.getY();
   ...
   */
   public boolean mouseDrag(MouseEvent event) // MouseEvent event 
   {
	   int x = event.getX();
	   int y = event.getY();
      if (Renderer.isDragging()) {
         theta += .03 * (double) (x - mx); // HORIZONTAL VIEW ROTATION .03
         phi += .03 * (double) (y - my); // VERTICAL VIEW ROTATION     .03
         //System.out.println(" x: "+theta+ " y: "+phi);
         mx = x;
         my = y;
   ...
   */
   public boolean processCommand(KeyEvent ke) {
	   int xx = ke.getKeyCode();
	      switch (xx) 
	      {
	      	 //case KeyEvent.VK_A :case KeyEvent.VK_B :case KeyEvent.VK_D :case KeyEvent.VK_S :case KeyEvent.VK_F :case KeyEvent.VK_R :
	      		 //cubo.mossa_da_tastiera(xx,maiuscolo);
	      		// return true;
	         case KeyEvent.VK_E :
	            renderer.showMesh = !renderer.showMesh;
	            damage();
   ...
   */
   ...
   protected class KeyHandler extends KeyAdapter {
      ...
      {
    	 rp.processCommand(ke);
      }
   }

	public void set_aliasing() 
	{
		aliasing = !aliasing;
		renderer.set_aliasing(aliasing);
